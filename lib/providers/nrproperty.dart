class NRProperty {

  DateTime date;
  int id;
  String slug;

  /// The title for the object.
  String title;

  /// The content for the object.
  String content;

  /// The ID for the author of the object
  //int author;

  /// The ID for the author of the object
  String author;

  List nrPropMeta;
  String nraddress;
  String nrsize;
  int nrprice;

  /// The excerpt for the object.
//  Map excerpt;

  /// The ID of the featured media for the object.
  int featuredMediaID;

  /// The URL of the featured media for the object.
  dynamic featuredMediaUrl;
  bool featuredMediaThumb;

  /// Whether or not comments are open on the object
  ///
  /// One of: open, closed
  //String commentStatus;

  /// Whether or not the object can be pinged.
  ///
  /// One of: open, close
  //String pingStatus;

  /// The format for the object.
  //String format;

  /// Meta fields.
  //dynamic meta;

  /// Whether or not the object should be treated as sticky.
  //bool sticky;

  /// The theme file to use to display the object.
//  /String template;

  /// The terms assigned to the object in the category taxonomy.
//  List<int> categories;

  /// The terms assigned to the object in the post_tag taxonomy.
//  List tags;

  // Injected objects
//  Media featuredMedia;
  //User user;
  NRProperty();

  NRProperty.fromMap(Map<String, dynamic> map) {
    if (map == null) {
      return;
    }

    date = map["date"] != null ? DateTime.parse(map["date"]) : null;
    id = map['id'];
    title = map['title']['rendered'];
    content = map['content']['rendered'];
//    author = map['author'];
    author = map["_embedded"]["author"][0]["name"];
    //excerpt = map['excerpt'];
    //featuredMediaID = map['featured_media'];
    //featuredMediaUrl = map ['_links']["self"][0]['href'] ;
    featuredMediaUrl = map["_embedded"]["wp:featuredmedia"] == null
        ? 'assets/images/placeholder.png'
        : map["_embedded"]["wp:featuredmedia"][0]["source_url"];

    nraddress = map["REAL_HOMES_property_id-description"]
                ["REAL_HOMES_property_location"] ==
            null
        ? 'No Address'
        : map["REAL_HOMES_property_id-description"]
            ["REAL_HOMES_property_location"][0];

    nrsize = (map["property_meta"]["REAL_HOMES_property_size"] != null)
        ? map["property_meta"]["REAL_HOMES_property_size"][0]
        : '';

    nrprice =
    (map["REAL_HOMES_property_id-description"]["REAL_HOMES_property_price"] ==
        null)
        ? 0
        : int.parse(
        map["REAL_HOMES_property_id-description"]["REAL_HOMES_property_price"][0]);

    // nrprice = map["property_meta"]["REAL_HOMES_agent"][0];
    //  post.featuredMediaUrl == null
    //           ? 'assets/images/placeholder.png'
    //           : post.featuredMediaUrl,
    // getFeaturedMedia(){
    //   if(featuredMediaThumb = true){
    //     return map["_embedded"]["wp:featuredmedia"][0]["source_url"] ;
    //   }else{
    //      return 'assets/images/placeholder.png' ;
    //   }
    // }
    // featuredMediaUrl = getFeaturedMedia();
    // print(featuredMediaUrl);

    //commentStatus = map['comment_status'];
    //pingStatus = map['ping_status'];
    //format = map['format'];
    //meta = map['meta'];
    //sticky = map['sticky'];
    //template = map['template'];
    //tags = map['tags'];

    // Avoiding (odd) cast warnings
//    categories = new List();
//    for (dynamic item in map['categories']) {
//      if (item is int) {
//        categories.add(item);
//      }
//    }
  }

  Map<String, dynamic> toMap() => {
        'date': date?.toIso8601String(),
//        'date_gmt': dateGMT?.toIso8601String(),
//        'guid': guid,
        'id': id,
//        'link': link,
//        'modified': modified?.toIso8601String(),
//        'modified_gmt': modifiedGMT?.toIso8601String(),
//        'slug': slug,
//        'status': status,
//        'type': type,
//        'password': password,
        'title': title,
        'content': content,
        'author': author,
        //'authorName':authorName,
//        'excerpt': excerpt,
        'featured_media': featuredMediaUrl,

//        'comment_status': commentStatus,
//        'ping_status': pingStatus,
//        'format': format,
//        'meta': meta,
//        'sticky': sticky,
//        'template': template,
//        'categories': categories,
//        'tags': tags
        'nraddress': nraddress,
        'nrsize': nrsize,
        'nrprice': nrprice
      };

  toString() => "NRProperty => " + toMap().toString();

  NRProperty.fromMapObject(Map<String, dynamic> map) {
    this.id = map['id'];
    this.title = map['title'];
    this.content = map['content'];
    this.author = map['author'];
    this.date = map["date"] != null ? DateTime.parse(map["date"]) : null;
    this.featuredMediaUrl = map['featured_media'];
    this.nraddress = map['nraddress'];
    this.nrsize = map['nrsize'];
    this.nrprice = map['nrprice'];
  }
}